...
async def midtrans_callback(request: Request):
    ...
    if not order_id_raw or not transaction_status:
        raise HTTPException(status_code=400, detail="order_id atau transaction_status tidak ditemukan di callback")
    ...
    # Jika sukses, update status order jadi 'paid'
    if transaction_status in ["settlement", "capture"]:
        supabase.table("orders").update({"status": "paid"}).eq("id", order_id_int).execute()
    return {"message": "Callback processed", "order_id": order_id_raw, "status": transaction_status}